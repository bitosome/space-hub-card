...
export class SpaceHubCard extends LitElement {
    ...
    if (config.headers && Array.isArray(config.headers)) {
      config.headers.forEach((header, index) => {
        ...
        if (header.weather) {
          ...
          if (weather.temperature_icon_count !== undefined && weather.temperature_icon_count !== null) {
          ...
          if (weather.metric_columns !== undefined && weather.metric_columns !== null) {
            const value = Number(weather.metric_columns);
            if (!Number.isFinite(value) || value < 1) {
              errors.push(`Header ${index + 1}: Weather tile metric_columns must be a positive number, got: ${weather.metric_columns}`);
            }
          }
  ...
  private _onSwitchAction(ev: CustomEvent, sw?: any): void {
    ...
    // HA owns confirmation and exemptions for every tile type. Do not start
    // optimistic feedback while a native dialog may still be awaiting consent.
    const prompt = normalizeConfirmation(selected.confirmation);
    const exempt = typeof prompt === 'object'
      && prompt.exemptions?.some((entry) => entry.user === this.hass?.user?.id);
    if (!prompt || exempt) this._trackPendingSwitch(entity, action, finalConfig);
